...
test("strategy card exposes operational metadata and preserves record actions", () => {
  ...
  assert.match(source, /onArchiveStrategy\?\.\(item\.id\)/);
  ...
  assert.match(source, /key: "archive", label: "Archive", variant: "secondary", hidden: !canArchive/);
  assert.match(source, /key: "delete", label: "Delete", variant: "danger"/);
});

test("strategy card offers Archive only for open records", () => {
  assert.match(source, /const canArchive = item\?\.status === "open"/);
});
