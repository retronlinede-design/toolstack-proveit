...
export default function SequenceGroupRecordManager({ group, groups, onOpenRecord, onOperation }) {
  ...
  return <div className="space-y-4">
    ...
    <div className="max-h-[42vh] space-y-2 overflow-y-auto pr-1">
      ...
      {visible.length === 0 && <p className="rounded-lg border border-dashed border-neutral-300 p-5 text-center text-sm text-neutral-500 dark:border-neutral-700">{records.length === 0 ? "This Issue currently contains no records." : "No records match these filters."}</p>}
    </div>
    {selected.size > 0 && <div className="space-y-3 rounded-xl border border-neutral-200 p-4 dark:border-neutral-700">
      ...
      <div className="flex flex-wrap gap-2"><select aria-label="Destination Issue" value={destination} onChange={(e) => setDestination(e.target.value)} className="min-h-11 flex-1 rounded-lg border border-neutral-300 bg-white px-3 dark:border-neutral-600 dark:bg-neutral-800"><option value="">Choose another Issue</option>{otherGroups.map((item) => <option key={item.name} value={item.name}>{item.name} ({item.totalCount})</option>)}</select><RecordActions actions={[{ key: "move", label: "Move to Issue", variant: "primary", onClick: moveExisting, disabled: !destination }]} /></div>
      <div className="grid gap-2 sm:grid-cols-2"><input aria-label="New Issue name" value={newGroup.name} onChange={(e) => setNewGroup({ ...newGroup, name: e.target.value })} placeholder="New Issue name" className="rounded-lg border border-neutral-300 bg-white px-3 py-2 dark:border-neutral-600 dark:bg-neutral-800" /><input aria-label="New Issue description" value={newGroup.description} onChange={(e) => setNewGroup({ ...newGroup, description: e.target.value })} placeholder="Description (optional)" className="rounded-lg border border-neutral-300 bg-white px-3 py-2 dark:border-neutral-600 dark:bg-neutral-800" /></div>
      <RecordActions actions={[{ key: "split", label: "Split Selected Records into New Issue", variant: "secondary", onClick: split }, { key: "remove", label: "Remove from Issue", variant: "danger", onClick: remove }]} />
