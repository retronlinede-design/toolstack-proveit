...
test("Next Actions is bounded to five daily-work items", () => { const actions = Array.from({ length: 7 }, (_, index) => ({ ...base.nextActions[0], id: `a${index}`, title: `Action ${index}` })); const html = render({ ...base, nextActions: actions }); assert.match(html, /Showing 5 of 7/); assert.match(html, /Action 4/); assert.doesNotMatch(html, /Action 5/); });
test("unresolved actions retain their text without a fake Open button", () => { const html = render({ ...base, nextActions: [{ ...base.nextActions[0], unavailable: true }] }); assert.match(html, /Review letter/); assert.match(html, /No direct record available/); });
