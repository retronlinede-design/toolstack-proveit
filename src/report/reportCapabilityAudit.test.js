...
test("active Report Centre capability matrix has no declared-to-rendered mismatch", () => {
  ...
  const result = auditReportCapabilities({
    ...
    runtimeDocuments: { client: {}, management: {}, investigation: {}, evidence: {}, document: {}, ledger: {}, caseAudit: {}, incidentSchedule: {}, chronologyReport: {}, action: {} },
    renderedActions,
    previewRenderers: Object.fromEntries(ids.map((id) => [id, true])),
    runtimeFormatters: { client: ["markdown", "json"], management: ["markdown", "json"], investigation: ["markdown", "json"], evidence: ["markdown", "json"], document: ["markdown", "json"], ledger: ["markdown", "json"], caseAudit: ["markdown", "json"], incidentSchedule: ["markdown", "json"], chronologyReport: ["markdown", "json"], action: ["markdown", "json"] },
  });
  assert.equal(result.valid, true);
  assert.deepEqual(result.warnings, []);
