...
test("legacy and metadata-only groups migrate deterministically with assignments", () => { const result = migrate(); assert.equal(result.caseData.issues.length, 2); assert.deepEqual(result.caseData.issues.map((x) => x.reference), ["ISS-001", "ISS-002"]); assert.equal(result.caseData.evidence[0].sequenceGroupId, result.caseData.incidents[0].sequenceGroupId); assert.equal(result.caseData.issues.find((x) => x.name === "Heating").description, "Legacy description"); });
...
test("metadata update stores owner date position and timestamp", () => { const base = migrate().caseData; const issue = base.issues[0]; const result = updateCaseIssue(base, issue.id, { status: "monitoring", priority: "high", ownerPartyId: "p1", reviewDate: "2026-08-15", currentPosition: "Awaiting response", purpose: "Track repairs" }, { now: "2026-08-01T00:00:00Z" }); assert.equal(result.success, true); assert.equal(result.issue.ownerPartyId, "p1"); assert.equal(result.issue.updatedAt, "2026-08-01T00:00:00Z"); });
test("rename preserves ID/reference and synchronizes compatibility labels", () => { const base = migrate().caseData; const issue = resolveCaseIssue(base, "Heating"); const result = renameCaseIssue(base, issue.id, "Winter Heating", { now: "2026-08-01T00:00:00Z" }); assert.equal(result.issue.id, issue.id); assert.equal(result.issue.reference, issue.reference); assert.equal(result.caseData.incidents[0].sequenceGroup, "Winter Heating"); assert.equal(result.caseData.incidents[0].sequenceGroupId, issue.id); });
