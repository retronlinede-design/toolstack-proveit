...
export function exportAllSequenceGroupAuditsMarkdown(caseData, options = {}) {
  ...
  return ["# All Issue Audits", "", `Case: ${md(report.case.name || report.case.id)}`, `Case reference: ${md(report.case.reference)}`, `Exported: ${report.exportedAt}`, `Issues: ${report.totals.sequenceGroups}`, `Assigned records: ${report.totals.groupedRecords}`, "", ...report.audits.flatMap((group) => [`## ${md(group.name)}`, `- State: ${group.state}`, `- Status: ${group.status}`, `- Description: ${md(group.description || "-")}`, `- Records: ${group.recordCount}`, `- Diagnostics: ${group.audit.threadOverview.diagnosticCount}`, `- Warnings: noIncidents=${group.warnings.noIncidents}; incidentsWithoutEvidence=${group.warnings.incidentsWithoutEvidence}`, `- Record types: ${Object.entries(group.recordTypeBreakdown).map(([type, count]) => `${type}=${count}`).join("; ")}`, "", "### Records", recordLines(group.records), "", "### Audit findings", ...diagnosticLines(group.audit.diagnostics), ""]),].join("\n");
}
...
export function exportCaseBySequenceGroupsMarkdown(caseData, options = {}) {
  ...
  return ["# Full Case by Issues", "", `Case: ${md(report.case.name || report.case.id)}`, `Case reference: ${md(report.case.reference)}`, `Description: ${md(report.case.description || "-")}`, `Exported: ${report.exportedAt}`, `Unique records: ${report.totals.uniqueRecords}`, `Assigned record appearances: ${report.totals.groupedRecordAppearances}`, `Unassigned records: ${report.totals.ungroupedRecords}`, `Record types: ${Object.entries(report.totals.recordTypes).map(([type, count]) => `${type}=${count}`).join("; ")}`, "", ...report.sequenceGroups.flatMap((group) => [`## ${md(group.name)}`, `- State: ${group.state}`, `- Status: ${group.status}`, `- Description: ${md(group.description || "-")}`, `- Records: ${group.records.length}`, "", recordLines(group.records), "", "### Audit findings", ...diagnosticLines(group.diagnostics), ""]), "## Unassigned Records", "", ...RECORD_TYPES.flatMap((type) => [`### ${type}`, recordLines(report.ungroupedRecords[type]), ""]), "## Unresolved and Missing References", "", ...(report.unresolvedReferences.length ? report.unresolvedReferences.map((item) => `- ${md(item.source)} | ${md(item.kind)} | ${md(item.message)} | technical reference: ${md(item.technicalReference)}`) : ["- None"]), ""].join("\n");
}
